...
var COUNT = OPT.count || 0;      // settled from the gallery's quality tier
/* Joint caps, jawlines and long hair read at arm's length, not at gallery
   distance. A modest device simply does without them: seven fewer pieces
   per figure, multiplied by everyone in the room. */
var FINE = false;
...
function buildFigure(){
  ...
  if (!drape) {
    [-1, 1].forEach(function(side){
      ...
      if (FINE) knee.add(part(geo.joint, leg, 0.068 * k, 0.062 * k, 0.068 * k, 0, 0, 0));
  ...
  [-1, 1].forEach(function(side){
    ...
    if (FINE) sh2.add(part(geo.joint, top, 0.055 * k, 0.05 * k, 0.055 * k, 0, 0, 0));
    ...
    if (FINE) el.add(part(geo.joint, skin, 0.042 * k, 0.045 * k, 0.042 * k, 0, -0.27 * k, 0));
...
function tick(dt){
  ...
  for (var i = 0; i < people.length; i++){
    ...
    /* the gallery may ask for a smaller crowd if the device is labouring */
    var cap = (G.peopleCap === undefined) ? people.length : G.peopleCap;
    var far = i >= cap ||
              Math.hypot(p.x - cam.position.x, p.z - cam.position.z) > FAR;
...
function init(){
  ...
  if (!COUNT) COUNT = (G.quality && G.quality.people) || (IS_TOUCH ? 7 : 17);
  FINE = !!(G.quality && G.quality.tier === 'high');
  makeGeometry();
  spawn();
  G.tick = tick;
  G.people = people;
